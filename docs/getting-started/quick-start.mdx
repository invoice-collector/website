...
- **Customer**: Refers to the company which has a contract with Invoice-Collector. _It should be you._
- **User**: Refers to a company interacting with Invoice-Collector on behalf of the customer (Company A, Company B, etc). _Users are your customers._
- **Credential**: Refers to the authentication information (such as username and password) required by the collector to download the user's invoices.
...
- A credential uses a collector to download the user's invoices.
...
We will start with a simple request to the endpoint [`GET /customer`](../api.mdx#tag/Customer/paths/~1customer/get) to retrieve your informations:
...
<Tabs groupId="server-base-url">
  <TabItem value="cloud" label="Cloud" default>
    ...
    You can update the `callback` url and other fields on the [Settings page](https://app.invoice-collector.com/settings).
  </TabItem>
  <TabItem value="self-hosted" label="Self-Hosted">
    ```bash
    ...
    ```json
    {
      ...
      "email": "account+dev@invoice-collector.com",                         // Email of your account
      "name": "default",                                                    // Name of your company
      "callback": "https://your.infrastructure.com/path/to/callback",       // Callback to which invoices will be sent
      ...
    }
    ```

    You can update the `callback` url and other fields with a request to the [`POST /customer`](../api.mdx#tag/Customer/paths/~1customer/post) endpoint.
  </TabItem>
</Tabs>

The `callback` field is the URL at which invoices will be sent. This callback URL is on your infrastructure and must accept POST requests with the invoice information in the request body.
See [`GET /test/callback/invoice`](../api.mdx#/paths/~1test~1callback~1{type}/get) for the request body schema.

We strongly recommend checking the API reference for the full response body. See [`GET /customer`](../api.mdx#tag/Customer/paths/~1customer/get).
...
You can make a request to the `POST /user` endpoint to create it in our database and generate the token.
...
<Tabs groupId="server-base-url">
  <TabItem value="cloud" label="Cloud" default>
  From the [Integrations page](https://app.invoice-collector.com/customer/integrations), click the "Test" button on the integration of your choice.
  </TabItem>
  <TabItem value="self-hosted" label="Self-Hosted">
    ```bash
    curl -X GET http://localhost:8080/api/v1/callback/{callback_id}/test/invoice \
        -H "Authorization: Bearer <bearer_token>"
    ```
  </TabItem>
</Tabs>
...
See the [API Reference for the callback schema](/docs/api.mdx#/paths/~1test~1callback~1{type}/get).
...
- Please refer to the [API Reference](/docs/api.mdx) for more advanced requests.
:::