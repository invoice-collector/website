...
<Tabs groupId="server-base-url">
  <TabItem value="cloud" label="Cloud" default>
    ...
    ```json
    {
      ...
      "callback": "https://path.to/callback",       // Callback to which invoices will be sent
      ...
    }
    ```
...
The `callback` field is the url at which invoices will be sent. This callback url must accept POST requests with the invoice informations in the request body.
See [`GET /test/callback/invoice`](../api.mdx#/paths/~1test~1callback~1{type}/get) for the request body schema.

We strongly recommend to check the API reference for the full response body. See [`GET /customer`](../api.mdx#tag/Customer/paths/~1customer/get).
