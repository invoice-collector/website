...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>2</td>
            <td>Frontend</td>
            <td>Update the iframe `href` to `/ui?token=${token}&credential_id=${credential_id}` and display the iframe. The iframe will get the current addition process and directly display the progress bar.</td>
        </tr>
    </tbody>
</table>

:::warning[Warning]
Calling `/ui?token=${token}&credential_id=${credential_id}` if a collect is not in progress will trigger a new collect.
Make sure the current [credential & collector states](../credential-and-collector-states) is between `0` and `7` excluded.
:::
