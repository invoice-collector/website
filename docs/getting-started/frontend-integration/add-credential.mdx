...
- Backend:
    - Call [`POST /user`](/docs/api.mdx#tag/User/paths/~1user/post) to create a user in our database and get the user
    - Call [`GET /collectors`](/docs/api.mdx#tag/Collector/paths/~1collectors/get) to list all the available collectors
    - Call [`POST /user/{user_id}/credential`](/docs/api.mdx#tag/Credential-(bearer)/paths/~1user~1{user_id}~1credential/post) to create a new credential
- Frontend:
