...
<table>
    ...
    <tbody>
        <tr>
            <td>1</td>
            <td>Frontend</td>
            <td>Create a hidden iframe with no `href`.</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Frontend</td>
            <td>User clicks on the "Add collector" button.</td>
        </tr>
        <tr>
            <td>3</td>
            <td>Frontend</td>
            <td>Update the iframe `href` to `/ui?token=${token}` and display the iframe.</td>
        </tr>
        <tr>
            <td>4</td>
            <td>Frontend</td>
            <td>Listen for events sent from the iframe to parent (see the next section).</td>
        </tr>
    </tbody>
</table>
...
| Panel Interactive | `{ type: 'ic-panel-interactive' }` | Current panel is "Canvas". User needs to authenticate using video stream. | ![iframe_big](../../diagrams/iframe/big.svg) |

In your application frontend, here is how the events can be catch:
...
window.addEventListener('message', function(event) {
    ...
    if (event.origin === "https://api.invoice-collector.com") {
        ...
        else if (event.data.type === 'ic-panel-interactive') {
            //Panel "Interactive" is displayed
...
<table>
    ...
    <tbody>
        <tr>
            <td>1</td>
            <td>Frontend</td>
            <td>Create a page to display all the available collectors.</td>
        </tr>
        <tr>
            <td>2</td>
            <td>Frontend</td>
            <td>Create a page to ask for credentials.</td>
        </tr>
        <tr>
            <td>3</td>
            <td>Frontend</td>
        ...
        <tr>
            <td>5</td>
            <td>Backend</td>
            <td>Call [`POST /user`](/docs/api.mdx#tag/User/paths/~1user/post) to create a user in our database and get the user.</td>
        </tr>
        <tr>
            <td>6</td>
            <td>Backend</td>
            <td>Call [`GET /collectors`](/docs/api.mdx#tag/Collector/paths/~1collectors/get) to list all the available collectors.</td>
        </tr>
        <tr>
            <td>7</td>
            <td>Backend</td>
            <td>Call [`POST /user/{user_id}/credential`](/docs/api.mdx#tag/Credential-(bearer)/paths/~1user~1{user_id}~1credential/post) to create a new credential.</td>
        </tr>
        <tr>
            <td>8</td>
            <td>Frontend</td>
